...
description:  "Build a simple MCP server in C# (.NET), expose tools over STDIO, and integrate it with VS Code’s GitHub Copilot using mcp.json."
...
cover:
    ...
    alt: "MCP server in .NET – cover image"
    caption: "Building a simple MCP server in C# (.NET)"
...
After creating the `.vscode/mcp.json` file and restarting or reloading VS Code if necessary, you will see an option to start the server as in the screenshot below.
![VS Code MCP server configuration UI showing simple-mcp-server](images/mcp.json.png)
...
![GitHub Copilot Chat in Agent mode within VS Code](images/copilot-agent-mode.png)
...
![MCP tools list showing ReverseString and CountWords available](images/mcp-server-tools.png)
...
- Tools don’t appear: Ensure the server launches (check terminal output) and that `mcp.json` path points to the correct `.csproj`.
- No Agent mode: Update Copilot extensions or enable preview/insiders features; restart VS Code.
- Server crashes on start: Verify `.NET 8 SDK` is installed and `ModelContextProtocol` package restored.
- Long‑running tool calls: Add timeouts or cancellation and watch the VS Code status bar for progress.

### D. Alternative Client: Claude Desktop (macOS/Windows/Linux)

Claude Desktop also supports MCP servers launched as subprocesses over STDIO. Configure a `claude_desktop_config.json` file and point it at your server.
  ...
  "mcpServers": {
    "simple-mcp-server": {
      ...
      "args": [
        "run",
        "--project",
        "/absolute/path/to/simple-mcp-server/simple-mcp-server.csproj"
      ],
      "env": {
...
Notes:
- Use absolute paths in `args` for reliability.
- After saving the config, restart Claude Desktop.
- Claude discovers and shows tools similarly to VS Code; look for your server name and tool list.

### E. Optional: Switch to HTTP + SSE Transport

STDIO is ideal for local development, but you can host your MCP server over HTTP and stream results via Server‑Sent Events (SSE) for remote clients. The exact API depends on the `ModelContextProtocol` SDK version you use, but the pattern is to swap the transport while leaving tools unchanged.

Program.cs (conceptual toggle):
...
var services = builder.Services
    .AddMcpServer()
    // For local development (child process over stdio):
    .WithStdioServerTransport()
    .WithTools<MyTools>();

// For HTTP + SSE hosting, consult your SDK docs; the transport swap typically looks like:
// services.WithHttpSseServerTransport(options =>
// {
//     options.BindAddress = "http://localhost:3000/mcp"; // endpoint path
// });
...
Key points:
- Transport swap should not change your tool implementations.
- See Part 2 of this series for transport and framing details: {{< relref "/blog/model-context-protocol/mcp-protocol-mechanics-and-architecture/" >}}.
- Use TLS for remote deployments and require authentication.
